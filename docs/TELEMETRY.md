...
| `llm.generate` | every LLM call | legacy executor label `provider`; bounded `executor`, underlying provider, model, and `locality` (`local` / `remote` / `unknown`) for routed calls. ACPX emits a known remote harness provider or the `unknown` sentinel, never its arbitrary configured agent string; `latencyMs`, `success`, `inputTokens`, `outputTokens`, `cacheReadTokens`, `cacheCreationTokens`, `totalTokens`, `totalCost`, `accountingProvenance` |
