...
async fn remaining_public_routes_have_contract_replay_coverage() {
    ...
    let resp = server.get("/api/repair/integrity-check").await;
    assert_status("GET /api/repair/integrity-check", &resp, &[200]);
    let body = server.json(resp).await;
    assert_eq!(body["ok"], true);
    assert_eq!(body["messages"], json!([]));
    let resp = server.post("/api/repair/rebuild-indexes", json!({})).await;
    assert_status("POST /api/repair/rebuild-indexes", &resp, &[200]);
    let body = server.json(resp).await;
    assert_eq!(body["integrity"]["ok"], true);
    assert!(body["fts"]["repaired"].is_boolean());
    assert!(body["fts"]["message"].is_string());
    assert!(body["embeddings"]["reembedded"].is_number());
    assert!(body["embeddings"]["totalMissing"].is_number());
