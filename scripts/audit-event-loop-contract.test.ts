...
test("the deterministic ledger retains the exact current source inventory", () => {
	...
	expect(baseline).toHaveLength(1001);
...
test("the generated report describes the type boundary and transitional counts", () => {
	...
	expect(report).toContain("Exact ledger inventory: 1001 sites");
	expect(report).toContain("66 synchronous writes, 109 synchronous reads, and 310 async-named parent DB sites");
