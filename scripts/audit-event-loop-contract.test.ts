...
test("the deterministic ledger retains the exact current source inventory", () => {
	...
	expect(baseline).toHaveLength(868);
	...
	expect(baseline.filter((site) => site.api === "withWriteTxAsync")).toHaveLength(41);
...
test("the generated report describes the type boundary and transitional counts", () => {
	...
	expect(report).toContain("Exact ledger inventory: 868 sites");
	expect(report).toContain("65 synchronous writes, 99 synchronous reads, and 189 async-named DB sites");
	expect(report).toContain("Async-named ON-PARENT DB sites: 187");
	expect(report).toContain("Async-named OFF-PARENT DB sites: 2");
	expect(report).not.toContain("async-named parent DB sites");
	expect(report).toContain(
		"The async-named DB counts above separate the 187 ON-PARENT callbacks from the 2 OFF-PARENT callbacks.",
	);
	expect(report).toContain("Database accessor sites classified: 353");
	expect(report).toContain("ON-PARENT callback execution: 351");
