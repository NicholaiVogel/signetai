...
function deadScore(dead: number, warn: number, fail: number): { penalty: number; status: QueueScore["status"] | null } {
...
function ageScore(age: number, warn: number, fail: number): { penalty: number; status: QueueScore["status"] | null } {
...
export function scoreCountsWithThresholds(
	...
	const bump = (candidate: "degraded" | "unhealthy" | null): void => {
