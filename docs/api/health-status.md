...
When the policy gate denies the action the response carries
`success: false` and HTTP `429` (cooldown active / hourly budget
exhausted / agents without `autonomous.enabled`). Wrong `action` value or
